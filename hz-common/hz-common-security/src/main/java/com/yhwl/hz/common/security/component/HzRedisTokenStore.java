 ...
 */
public class HzRedisTokenStore implements TokenStore {
	...
	public HzRedisTokenStore(RedisConnectionFactory connectionFactory, KeyStrResolver resolver) {
	...
	public long doMaintenance() {
		...
		try (RedisConnection conn = getConnection()) {
			...
			Cursor<byte[]> unameToAccessKeys = conn.scan(ScanOptions.scanOptions()
					.match(keyStrResolver.extract(prefix + UNAME_TO_ACCESS + "*", StrUtil.COLON)).build());
