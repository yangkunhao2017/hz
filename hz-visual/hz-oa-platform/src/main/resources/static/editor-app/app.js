  ...
  .run(['$rootScope', '$timeout', '$modal', '$translate', '$location', '$window', '$http', '$q',
        function($rootScope, $timeout, $modal, $translate, $location, $window, $http, $q) {
            ...
            function fetchModel(modelId) {
                var tenantId  = 1
                if (localStorage.getItem('hz-tenantId')) {
                  tenantId = JSON.parse(localStorage.getItem('hz-tenantId')).content
                }

                var token = ''
                if (sessionStorage.getItem('hz-access_token')){
                  token = JSON.parse(sessionStorage.getItem('hz-access_token')).content
                }
